    ...
    async _handleKeyboard(event) {
        ...
        if (isCopy && event.action === "keydown") {
            try {
                ...
                if (selectedText) {
                    this.io.emit("clipboard-copy", selectedText);
                }
            } catch (err) {
        ...
        if (event.isChar && event.action === "keydown") {
            ...
            if (event.action === "keydown") await this.page.keyboard.down(event.key).catch(() => {});
            if (event.action === "keyup") await this.page.keyboard.up(event.key).catch(() => {});
        }
    }
